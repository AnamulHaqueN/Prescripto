...
//               {!item.cancelled && item.payment && !item.isCompleted && (<button className="text-sm text-center sm:min-w-48 py-2 bg-green-500 hover:bg-green-600 text-white rounded transition-all duration-300">Paid</button>)}
//               {!item.cancelled && !item.payment && !item.isCompleted && (<button onClick={() => handlePayment(item._id)} className="text-sm text-stone-500 text-center sm:min-w-48 py-2 hover:bg-primary hover:text-white transition-all duration-300">Pay Online</button>)}
        ...
        {appointments.map((item, index) => (
          ...
          >
            ...
            <div className="flex-1 text-sm text-zinc-600 ">
              ...
              {item.isEmergency && (
                <span className="text-xs text-red-600 font-semibold bg-red-100 px-2 py-1 rounded inline-block mt-2">
                  EMERGENCY
                </span>
              )}
            </div>
