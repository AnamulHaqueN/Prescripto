...
const MyAppointments = () => {
  ...
  const getUserAppointments = async () => {
    try {
      ...
      if(data.success) {
        setAppointments(data.appointments.reverse()) // reverse for new appointment at the top
    ...
    <div>
      ...
      <div>
        {appointments.map((item, index)=>(
          <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-2 ' key={index}>
            <div>
            ...
            <div className='flex-1 text-sm text-zinc-600 '>
              ...
              <p><span className='text-sm text-neutral-700 font-medium '>Date & Time:</span> {slotDateFormat(item.slotDate)} | {item.slotTime}</p>
            </div>
            <div></div>
